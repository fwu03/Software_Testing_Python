...
def test_output():
    assert mlist.flatten_list_prime.flatten_list_prime([[12, 5], [2, 3]]) == [2, 3, 5] # assert return contain correct values
    assert isinstance(mlist.flatten_list_prime.flatten_list_prime([[12, 5], [2, 3]]), list) # assert return is a list
    assert len(mlist.flatten_list_prime.flatten_list_prime([[12, 5], [2, 3]])) >= len([[12, 5], [2, 3]])  # assert return length is less than or equal to input length
